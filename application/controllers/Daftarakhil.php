...
class Daftar extends CI_Controller {
	...
	public function index()
	{
		...
		if($valid->run()===FALSE) {
			//End Validasi
		$data = array( 'title'	=> 'Registrasi Pelapak',
						'isi'	=> 'daftar/list'
					);
		...
		$data = array( 	'status_pelapak'	=> 'Pending',
    ...
    public function _sendEmail($token, $test)
    {
        ...
        $this->email->from('websebar@gmail.com', 'Sebar Web (Sewa Barang');
        $this->email->to($this->input->post('email_pelapak'));

      	if ($test == 'verify') {
            $this->email->subject('Account Verification');
            $this->email->message('Klik link dibawah ini untuk verifikasi akun: <a href="'. base_url() . 'Daftar/verify?email_pelapak=' . $this->input->post('email_pelapak') . '&token=' . urlencode($token) . '">Activate</a>');
        } else if ($test == 'forgot') {
            $this->email->subject('Reset Password');
            $this->email->message('Click this link to reset your password : <a href="'. base_url() . 'Daftar/resetPassword?email_pelapak=' . $this->input->post('email') . '&token=' . urlencode($token) . '">Reset Password</a>');
        }
